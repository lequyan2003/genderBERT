...
from majority_voting import mv, mv_stats_f1
...
def create_model(preload_model, model_type, bert_freeze):
    ...
    if bert_freeze:
        ...
        for name, param in model.named_parameters():
            if (
                "classifier" not in name and "linear" not in name
            ):  # classifier layer
...
def train_epoch(model, data_loader, optimizer, device, scheduler):
    ...
    for _, batch in enumerate(data_loader):
        ...
        outputs = model(
            b_input_ids, labels=b_labels, attention_mask=b_input_mask
        )
        loss = outputs[0]
...
def eval_model(model, data_loader, device, model_type):
    ...
    for batch in data_loader:
        ...
        with torch.no_grad():
            outputs = model(
                b_input_ids, attention_mask=b_input_mask, labels=None
            )
